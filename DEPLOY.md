...
### 3. Подготовка файлов базы данных

Перед первым запуском убедитесь, что файлы `database.db` и `config.ini` существуют как файлы, а не каталоги. Если Docker ранее создал каталоги вместо файлов, исправьте это:

```bash
# Исправление проблемы с каталогами (если нужно)
./fix-db-files.sh

# Или используйте скрипт деплоя, который автоматически исправит проблему
./deploy.sh
```
...
**Важно:** Если при первом запуске `docker-compose up -d` Docker создал каталоги `database.db` или `config.ini` вместо файлов, удалите их и запустите скрипт `./fix-db-files.sh` для автоматического создания правильных файлов.
